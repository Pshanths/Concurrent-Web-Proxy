...
/*
Using the professors solution for the sequential proxy, I moved everything
after the socket accepts a connection into a different function called prxy.
This function will handle the process connection. This is the function that is called when a 
thread is created so each thread can handle their own respective clients.
A data structure was created because only one argument can be passed to the prxy function under pthread_create.
 Therefore the arguments needed (clientaddr, connfd) were put into the datastructure and the datastructure was passed instead (I learned this through google).
A mutex lock is used when writing into the log file so only one thread can acess it at a time.
*/
...
pthread_mutex_t texmex;    /* mutex for log file */
...
struct pcreatestruct
{
  struct sockaddr_in clientaddr;
  int connfd;
};
typedef struct pcreatestruct fddata;

// Process the rest of the connection after ACCEPT (just copy pasted from professors sequential solution onto here with minor changes)
void *prxy(fddata *input)
{
   ...
   //Used to fix a bug
    ...
    fddata *aargs = (fddata *)input; /* Get the clientaddr,connfd from the main function */
    ...
    realloc_factor = 2;
    request_len = 0;
    Rio_readinitb(&rio, aargs->connfd);
    while (1) {
	...
	response_len += n;
	Rio_writen_w(aargs->connfd, buf, n);
#if defined(DEBUG)	
     ...
     */
    pthread_mutex_lock(&texmex); // Mutex lock for proper log file writing
    format_log_entry(log_entry, &aargs->clientaddr, request_uri, response_len);  
    fprintf(log_file, "%s %d\n", log_entry, response_len);
    fflush(log_file);
    pthread_mutex_unlock(&texmex); //Mutex unlock
...
int main(int argc, char **argv)
{
    ...
    while (1) { 
      /* Initialize Struct for pthread arguments */
      fddata *aargs = (fddata *)malloc(sizeof(aargs));
      error = 0;   //Used to fix a bug
      clientlen = sizeof(clientaddr);  
      connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);
	
      
      /* move function arguments into struct */
      aargs->clientaddr = clientaddr;
      aargs->connfd = connfd;
   
      /* Create threads and make them use prxy function */
      pthread_create(&tid, 0, prxy, aargs);
      pthread_detach(tid);
    }
