...
This function will handle the process connection. This is the function that is called
after forking so each child process can handle their own connection. This was taken from 
slide 7 in the processes ppt under sakai resources.
A semaphore is used when writing into the log file so only one process can acess it at a time.
 */
...
sem_t locker; /* semaphore for log file */
...
void *prxy(int confd,struct sockaddr_in caddr)
{
   ...
   //Used to fix a bug
     ...
     */
    P(&locker); //lock log file
    format_log_entry(log_entry, &caddr, request_uri, response_len);  
    fprintf(log_file, "%s %d\n", log_entry, response_len);
    fflush(log_file);
    V(&locker); //unlock log file
...
int main(int argc, char **argv)
{
	...
	{ /*child process */
	  ...
	  prxy(connfd,clientaddr); /* process the request */
	  exit(0);
	}
      Close(connfd);
